...
for (const key of canonicalKeys) {
  ...
  assert.ok(["not_confirmed", "provider_confirmed", "vezunde_verified"].includes(definition.default_confirmation_level), `Confirmation default invalid pentru ${key}`);
...
assert.equal(isServicePubliclyEligible(medicalProviderConfirmed, claimedLocation), true, "Serviciul medical declarat de furnizor trebuie să fie public fără verificare separată");
assert.equal(isServiceMatchingEligible(medicalProviderConfirmed, claimedLocation), true, "Serviciul medical declarat de furnizor trebuie să poată intra în matching");
