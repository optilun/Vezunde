...
assert.match(workspace, /status\?\.contact_phone_available === true/);
assert.match(lifecycleComponent, /Cererea a fost rezolvată/);
assert.match(lifecycleComponent, /Închide cererea/);
assert.doesNotMatch(component, /response\.contact_phone|contact_phone\s*:|original_message|detailed_message|responder_user_id/);
assert.doesNotMatch(workspace, /response\.contact_phone|contact_phone\s*:|access_token_hash|responder_user_id/);
