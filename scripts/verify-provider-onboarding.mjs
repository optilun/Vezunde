...
const checks = [
  ...
  [workspaceBackend.includes("subjectType === 'independent_professional'"), 'professional applicants cannot edit unsupported location drafts'],
  [workspaceBackend.includes('B2B_CLAIM_PREP_ALLOWED_SECTIONS'), 'B2B preparation is limited to the public profile'],
  [workspaceBackend.includes('submitted.claim_subject_type || claim.claim_subject_type'), 'payload metadata overrides legacy schema compatibility fields'],
  ...
  [submitBackend.includes('schemaCompatibleSubjectType'), 'B2B requests remain compatible with the current claim schema'],
  [submitBackend.includes('request_type: requestType'), 'request type is persisted in the review payload'],
  ...
  [reviewBackend.includes("verification_status: 'approved'"), 'approval status is persisted in the compatible review payload'],
