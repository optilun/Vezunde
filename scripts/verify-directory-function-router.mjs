...
assert.equal(logicalNames.length, 20, 'Contractul directory trebuie sa pastreze exact cele 20 de nume logice consolidate (19 + adminFragmentedOrganizations, adaugat 2026-08-19 pentru detectia si fuziunea organizatiilor duplicate)');
...
assert.match(bridgeEntrySource, /viasee-directory-import-single-file-15/);
assert.match(bridgeEntrySource, /directory-import-runtime-national-directory-5/);
...
assert.match(bridgeEntrySource, /rejects_ambiguous_organization_match:\s*true/);
assert.match(bridgeEntrySource, /chunked_snapshot_finalization:\s*true/);
assert.match(bridgeEntrySource, /chunked_batch_planning:\s*true/);
assert.match(bridgeEntrySource, /reconciles_existing_location_metadata:\s*true/);
...
assert.match(bridgeEntrySource, /automated_controlled_import_orchestrator:\s*true/);
assert.match(bridgeEntrySource, /scheduled_auto_import_runner:\s*true/);
assert.match(bridgeEntrySource, /scheduled_runner_uses_cron:\s*true/);
assert.match(bridgeEntrySource, /accepts_multiple_automation_payload_shapes:\s*true/);
assert.match(bridgeEntrySource, /browser_watchdog_fallback:\s*true/);
assert.match(bridgeEntrySource, /max_automatic_execution_chunk:\s*5/);
assert.match(bridgeEntrySource, /supports_private_zip_upload:\s*true/);
assert.match(bridgeEntrySource, /persists_approved_source_subset:\s*true/);
assert.match(bridgeEntrySource, /uses_chunked_private_payload_storage:\s*true/);
assert.match(bridgeEntrySource, /repairs_partial_preflight_runs:\s*true/);
assert.match(bridgeEntrySource, /supports_national_directory_campaign:\s*true/);
assert.match(bridgeEntrySource, /publishes_unclaimed_unverified_directory_profiles:\s*true/);
assert.match(bridgeEntrySource, /excludes_controlled_profiles_from_national_import:\s*true/);
assert.match(bridgeEntrySource, /publishes_basic_directory_details_only_when_quality_allows:\s*true/);
assert.match(bridgeEntrySource, /preserves_top_three_isolation:\s*true/);
