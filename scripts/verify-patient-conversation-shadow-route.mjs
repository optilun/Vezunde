...
assert(wrapperSource.includes("const evaluationCaseId = String(source.evaluation_case_id ?? '').trim();"));
...
assert(wrapperSource.includes('function normalizeNonInvokedRuntimeIdentity('));
assert(wrapperSource.includes("envelope?.reason !== 'user_message_required'"));
assert(wrapperSource.includes('operational?.model_calls_used !== 0'));
