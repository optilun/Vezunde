...
Deno.serve(async (req) => {
  try {
    ...
    const ownerOrganizationIdsAll = [...new Set(own
      .filter((membership) => role(membership.role) === 'organization_owner')
      .map((membership) => organizationIdByMembership.get(membership.id))
      .filter(Boolean))];
    if (requestedOrganizationId && !ownerOrganizationIdsAll.includes(requestedOrganizationId) && user.role !== 'admin') {
      return res({ error: 'Doar ownerul organizatiei poate gestiona utilizatorii' }, 403);
    }
    ...
    const ownerOrganizationIds = requestedOrganizationId ? [requestedOrganizationId] : ownerOrganizationIdsAll;
    const ownerLocationIds = [...new Set(scopedOwn.filter((membership) => role(membership.role) === 'organization_owner').map((membership) => membership.location_id))];
    ...
    const members = [];
    for (const membership of visibleRows.values()) members.push(safeMembership(membership, await userInfo(svc, membership.user_id), organizationIdByLocation.get(membership.location_id)));
    const pendingInvitations = canManage
      ? (await svc.entities.ProviderMemberInvitation.filter({ status: 'pending' }, '-created_date', 500).catch(() => []))
        ...
        .map(safeInvitation)
      : [];
    const roleByLocation = Object.fromEntries(assigned.map((id) => [id, highest(scopedOwn.filter((membership) => membership.location_id === id).map((membership) => role(membership.role)))]));
