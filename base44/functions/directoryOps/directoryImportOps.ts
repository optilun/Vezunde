...
function executionProgressFromRows(rows, mutations = []) {
  ...
  for (const row of rows) {
    if (row.status === 'applied') {
      ...
      totals.execution_cursor += 1;
    } else if (row.status === 'failed' && !isTransientDirectoryExecutionFailure(new Error(clean(row.error_message, 1200)))) {
...
async function persistBatchInterruption(svc, batch, progress, error) {
  ...
  const executionCounts = {
    ...
    updated_organizations: Number(previousSummary.execution_counts?.updated_organizations || 0)
      + Number(progress.updatedOrganizations || 0),
  };
  try {
    await svc.entities.DirectoryImportBatch.update(batch.id, {
...
async function resumeBatchAfterTransientFailure(svc, user, input) {
  ...
  const failureMessage = remainingTransientRows > 0
    ? `${remainingTransientRows} randuri intrerupte temporar mai trebuie pregatite pentru reluare.`
    : '';
