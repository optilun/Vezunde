...
const MEMBERSHIP_ROLES = ['organization_owner', 'location_manager', 'location_staff'];
const VERIFICATION_METHODS = ['manual_review', 'official_email', 'public_phone', 'existing_owner_approval'];
...
Deno.serve(async (req) => {
  try {
    ...
    if (!p.representation_confirmed) return Response.json({ error: 'Confirmarea reprezentarii este obligatorie' }, { status: 400 });
    const c = p.contact || {};
    if (!String(c.contact_name || '').trim() || !String(c.email || '').trim()) {
    ...
    if (!RELATIONSHIPS.includes(claimantRelationship)) {
      return Response.json({ error: 'Selecteaza relatia ta cu organizatia sau locatia' }, { status: 400 });
    }
    const requestedMembershipRole = ROLE_BY_RELATIONSHIP[claimantRelationship];
    if (!MEMBERSHIP_ROLES.includes(requestedMembershipRole)) {
      return Response.json({ error: 'Rolul solicitat nu poate fi determinat' }, { status: 400 });
    }
    if (p.requested_membership_role && p.requested_membership_role !== requestedMembershipRole) {
      return Response.json({ error: 'Rolul solicitat nu corespunde relatiei declarate' }, { status: 400 });
    }
    const verificationMethod = normalizeVerificationMethod(p.verification_method || c.verification_method);
    ...
    if (p.mode === 'claim') {
      ...
      if (loc.status !== 'publicata' || loc.active_status === 'inactiva' || (loc.profile_control_status || 'directory') === 'suspended') {
        return Response.json({ error: 'Aceasta locatie nu poate primi solicitari de acces momentan.' }, { status: 400 });
      }
      organizationId = loc.organization_id || null;
      businessName = loc.public_display_name || loc.name;

      const previousUserClaims = await svc.entities.ProviderClaimRequest.filter({ location_id: locationId, user_id: user.id }, '-created_date', 20).catch(() => []);
      const activeOwnClaim = previousUserClaims.find((claim) => ACTIVE_CLAIM_STATUSES.includes(claim.status));
      if (activeOwnClaim) return Response.json({ error: 'Ai deja o solicitare pentru aceasta locatie. O poti urmari din contul tau.' }, { status: 400 });
      const approvedOwnClaim = previousUserClaims.find((claim) => claim.status === 'aprobata');
      if (approvedOwnClaim) return Response.json({ error: 'Ai deja o solicitare aprobata pentru aceasta locatie.' }, { status: 400 });

      const activeMemberships = await svc.entities.ProviderMembership.filter({ location_id: locationId, status: 'active' }, '-created_date', 50).catch(() => []);
      if (activeMemberships.some((membership) => membership.user_id === user.id)) {
        return Response.json({ error: 'Ai deja acces la administrarea acestei locatii.' }, { status: 400 });
      }

      const controlled = activeMemberships.length > 0 || CONTROLLED_PROFILE_STATUSES.includes(loc.profile_control_status || '') || loc.claim_verification_status === 'approved';
      requestType = controlled ? 'access_request_existing_claimed_profile' : 'claim_existing_directory_profile';
      if (!controlled) await svc.entities.ProviderLocation.update(locationId, { claim_verification_status: 'pending' });
      ...
      if (!String(l.name || '').trim() || !l.provider_type) return Response.json({ error: 'Numele profilului si tipul sunt obligatorii' }, { status: 400 });
      if (!PROFILE_TYPES.includes(l.provider_profile_type)) return Response.json({ error: 'Tipul profilului lipseste sau este invalid' }, { status: 400 });
      ...
      if (claimSubjectType !== 'b2b_supplier' && B2B_PROFILE_TYPES.includes(l.provider_profile_type)) {
        return Response.json({ error: 'Tipul B2B trebuie trimis prin fluxul de parteneri' }, { status: 400 });
      }
      if (!String(l.address || '').trim()) return Response.json({ error: 'Adresa este obligatorie' }, { status: 400 });
      const phonePublic = String(l.phone_public || '').trim();
      const publicEmail = String(l.public_email || '').trim();
      if (!phonePublic && !publicEmail) return Response.json({ error: 'Este necesar cel putin un contact public: telefon sau email' }, { status: 400 });
      ...
      if (claimSubjectType === 'independent_professional') {
        if (!String(prof.full_name || '').trim()) return Response.json({ error: 'Numele profesionistului este obligatoriu' }, { status: 400 });
        if (!PROFESSIONAL_TYPES.includes(prof.professional_type)) return Response.json({ error: 'Tipul de profesionist lipseste sau este invalid' }, { status: 400 });
      }

      const sirutaCode = String(l.locality_siruta_code || '').trim();
      if (!sirutaCode) return Response.json({ error: 'Selectarea localitatii din lista oficiala este obligatorie' }, { status: 400 });
      const geoRows = await svc.entities.GeographicLocality.filter({ siruta_code: sirutaCode, is_active: true });
      const geo = geoRows[0];
      if (!geo) return Response.json({ error: 'Localitatea selectata nu este valida' }, { status: 400 });
      ...
      identitySnapshot = JSON.stringify({
        ...
        source_flow: claimSubjectType === 'organization' ? 'organization_onboarding_v2' : claimSubjectType === 'b2b_supplier' ? 'b2b_onboarding_v2' : 'professional_onboarding_v2',
      ...
      requestType = claimSubjectType === 'b2b_supplier'
        ? 'new_b2b_supplier_profile'
        : claimSubjectType === 'independent_professional'
          ? 'new_professional_profile'
          : 'new_patient_facing_location';
      const reviewSnapshot = {
        ...
        professional_identity: claimSubjectType === 'independent_professional' ? { full_name: prof.full_name, professional_type: prof.professional_type } : null,
        request_type: requestType,
        proposed_location: {
          name: l.name,
      ...
      if (identityBlocking === 'strong_duplicate_review_required') {
        if (p.escalate_duplicate_review === true) {
          if (identityNote.length < 15) return Response.json({ error: 'Explica pe scurt de ce este o locatie diferita (minim 15 caractere)' }, { status: 400 });
        ...
        return Response.json({ identity_check: { ...identity, message: 'Am gasit un profil foarte asemanator. Verifica daca este deja profilul potrivit.' } });
      }
      if (identityBlocking === 'warning' && identityNote.length < 15) return Response.json({ identity_check: identity });
      ...
      const locData = {
        ...
        profile_control_status_reason: claimSubjectType === 'b2b_supplier' ? 'Partener B2B trimis spre verificare' : claimSubjectType === 'independent_professional' ? 'Profil profesional trimis spre verificare' : 'Locatie noua trimisa spre verificare',
