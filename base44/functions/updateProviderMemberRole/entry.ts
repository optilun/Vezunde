...
Deno.serve(async (req) => {
  try {
    ...
    if (role(m.role) === 'organization_owner' && proposed !== 'organization_owner' && await ownerCount(svc, m.location_id, m.id) === 0) return res({ error: 'Nu poti elimina ultimul owner activ al locatiei' }, 400);
