...
const ProviderLocationsWithPhoto = lazy(
  () => import("./ProviderLocationsWithPhoto"),
);
const ProviderLocationModulePage = lazy(
  () => import("./ProviderLocationModulePage"),
);
...
function organizationContextsFor(workspace) {
  ...
  const organizations = workspace.organizations?.length
    ...
    : [
        ...new Set(
          locations.map((location) => location.organization_id).filter(Boolean),
        ),
      ].map((id) => ({
        id,
        name:
          locations.find((location) => location.organization_id === id)
  ...
  return organizations.map((organization) => {
    ...
    const locationIds = new Set(
      contextLocations.map((location) => location.id),
    );
    const memberships = (workspace.memberships || []).filter((membership) => {
      const organizationId =
        membership.organization_id ||
        locationById[membership.location_id]?.organization_id;
      return (
...
export default function ProviderWorkspaceRoot({
  ...
  const requestedLocationModule = LOCATION_MODULES.has(locationModule)
    ? locationModule
    : null;
  ...
  const initialLocationId = allLocations.some(
    ...
    : allLocations.some((location) => location.id === requestedLocationId)
      ...
      : requestedOrganizationLocationId ||
        (allLocations.some(
          (location) => location.id === preferredProviderLocationId,
        )
          ? preferredProviderLocationId
          : workspace.memberships?.[0]?.location_id ||
            allLocations[0]?.id ||
            "");
      ...
      organizationContexts.find(
        (context) =>
          ...
          context.memberships?.some(
            (membership) => membership.location_id === selectedLocationId,
          ),
      ) ||
      organizationContexts[0] ||
      null,
    [organizationContexts, selectedLocationId],
  );
  const selectedOrganizationId = selectedContext?.organization?.id || "";
  const locations = selectedContext
    ? selectedContext.locations || []
    : allLocations;
  const memberships = selectedContext
    ? selectedContext.memberships || []
    : workspace.memberships || [];
  const capabilities = new Set(selectedContext?.capabilities || []);
  const canManageOrganizationProfile = capabilities.has(
    "organization.manage_profile",
  );
  ...
  const canManageMembers = Boolean(
    selectedContext?.can_manage_members ||
      capabilities.has("organization.manage_members"),
  );
  const canManageSettings = Boolean(
    selectedContext?.can_manage_settings ||
      capabilities.has("organization.manage_settings"),
  );
  const activeLocationModule =
    requestedLocationModule &&
    capabilities.has(LOCATION_MODULE_CAPABILITIES[requestedLocationModule])
      ? requestedLocationModule
      : null;
  const deniedLocationModule = Boolean(
    requestedLocationModule && !activeLocationModule,
  );
  const hasOwnerAccess = organizationContexts.some(
    (context) =>
      context.can_manage_members ||
      context.can_manage_settings ||
      context.current_user_role === "organization_owner",
  );
  const scopedWorkspace = useMemo(
    () => ({
      ...workspace,
      organizations: selectedContext?.organization
        ? [selectedContext.organization]
        : workspace.organizations,
      locations,
      memberships,
      current_user_role:
        selectedContext?.current_user_role || workspace.current_user_role,
      current_user_capabilities:
        selectedContext?.capabilities ||
        workspace.current_user_capabilities ||
        [],
  ...
  const loadOverview = async (locationId, options = {}) => {
    ...
    const response = await base44.functions
      .invoke("getProviderWorkspaceOverview", { location_id: locationId })
      .catch(() => ({ data: null }));
  ...
  const refreshOverviewInPlace = () =>
    loadOverview(selectedLocationId, { silent: true });

  useEffect(() => {
    if (ownerSyncStarted.current || !hasOwnerAccess) return;
    ownerSyncStarted.current = true;
    base44.functions
      .invoke("syncProviderOrganizationOwnerAccess", {})
      .then((response) => {
        if (response.data?.changed) onRefresh?.();
      })
      .catch(() => null);
  }, [hasOwnerAccess, onRefresh]);

  useEffect(() => {
    const routeLocationExists = allLocations.some(
      (location) => location.id === routeLocationId,
    );
  ...
  useEffect(() => {
    const requestedLocationExists = allLocations.some(
      (location) => location.id === requestedLocationId,
    );
    const nextLocationId = requestedLocationExists
      ? requestedLocationId
      : requestedOrganizationLocationId;
  ...
  useEffect(() => {
    ...
    if (selectedLocationId)
      rememberProviderLocation(user?.id, selectedLocationId);
  }, [selectedLocationId]);

  useEffect(() => {
    const denied =
      deniedLocationModule ||
    ...
    if (denied) {
      routerNavigate(
        deniedLocationModule
          ? "/contul-meu?s=locations"
          : "/contul-meu?s=overview",
        { replace: true },
      );
    }
  }, [
  ...
  const selectLocation = (locationId) => {
    ...
    if (activeLocationModule)
      routerNavigate(
        `/contul-meu/locatii/${locationId}/${activeLocationModule}`,
      );
  };

  const selectOrganization = (organizationId) => {
    const context = organizationContexts.find(
  ...
  const openLocationModule = (moduleKey, locationId = selectedLocationId) => {
    if (
      !LOCATION_MODULES.has(moduleKey) ||
      !locationId ||
      !capabilities.has(LOCATION_MODULE_CAPABILITIES[moduleKey])
    )
      return;
    setSelectedLocationId(locationId);
  ...
  const normalizedSection = ["services", "team", "hours", "photos"].includes(
    requestedSection,
  )
    ? "locations"
    : requestedSection;
  ...
  const selectedStatus =
    overview?.location?.id === selectedLocationId
      ? overview.location.profile_control_status
      : "";
  const statusLabel = selectedStatus
    ? PROFILE_CONTROL_LABELS[selectedStatus] || selectedStatus
    : "";
  const statusGreen = ["verified", "claimed"].includes(selectedStatus);
  const activeLocationCount = locations.filter(
    (location) =>
      location.active_status !== "inactiva" && location.status !== "suspendata",
  ).length;
  const multiLocation = activeLocationCount >= 2;
  const organizationName =
    selectedContext?.organization?.public_display_name ||
  ...
  return (
    <ProviderAppShell
      ...
      publicProfileUrl={
        selectedLocationId ? `/furnizor/${selectedLocationId}` : null
      }
      modeSwitches={modeSwitches}
      statusBadge={
        statusLabel || multiLocation ? (
          <span className="hidden items-center gap-1.5 sm:inline-flex">
            {statusLabel && (
              <span
                className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${statusGreen ? "bg-green-100 text-green-800" : "bg-secondary text-foreground"}`}
              >
                {statusLabel}
              </span>
            )}
            {multiLocation && (
              <span className="rounded-full bg-secondary px-2 py-0.5 text-[11px] font-semibold text-foreground">
                Organizație cu {activeLocationCount} locații
              </span>
            )}
          </span>
        ) : null
    ...
    >
      {(organizationContexts.length > 1 ||
        (memberships.length > 1 && safeSection !== "settings")) && (
        <LocationSwitcher
