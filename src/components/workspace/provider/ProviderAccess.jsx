...
function groupRoleLabel(group) {
  ...
  if (roles.length === 0) return "Fără rol activ";
  if (roles.length > 1) return "Roluri diferite pe locații";
...
function accessSummary(group, allLocationIds, locationById) {
  ...
  if (ownerEverywhere) return { label: "Întreaga organizație", locations: [] };
  ...
  return (
    <div className={`rounded-2xl border p-3.5 transition ${selected ? "border-foreground/20 bg-secondary/35" : "border-border bg-card"}`}>
      <div className="flex items-start gap-3">
        <button type="button" disabled={disabled} onClick={onToggle} className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md border disabled:opacity-50 ${selected ? "border-foreground bg-foreground text-background" : "border-border bg-background"}`} aria-label={selected ? "Elimină locația" : "Adaugă locația"}>
          {selected && <Check className="h-3.5 w-3.5" />}
        </button>
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-bold">{locationName(location)}</div>
          <div className="mt-0.5 truncate text-xs text-muted-foreground">{location.locality_name || location.city || "Localitate lipsă"}</div>
        </div>
      </div>
      {selected && showRole && (
        <div className="mt-3 border-t border-border/70 pt-3">
          <label className="text-xs font-semibold text-muted-foreground">Rol în această locație</label>
...
export default function ProviderAccess({ organizationId = "", locations = [] }) {
  ...
  const saveMemberAccess = async () => {
    ...
    if (!window.confirm(assignments.length > 0 ? "Salvezi accesul acestui utilizator?" : "Elimini accesul utilizatorului din toate locațiile?")) return;
    setSaving(true);
    setMessage("");
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1.5 max-w-3xl text-sm leading-relaxed text-muted-foreground">Gestionează membrii organizației și accesul lor la una, mai multe sau toate locațiile actuale.</p>
            </div>
          </div>
          {data?.can_manage_members && <button type="button" onClick={openInvite} className="inline-flex h-11 items-center gap-2 rounded-full bg-foreground px-5 text-sm font-semibold text-background hover:opacity-90"><UserPlus className="h-4 w-4" /> Invită utilizator</button>}
        </div>
      ...
      <section className="overflow-hidden rounded-[20px] border border-foreground/10 bg-card shadow-[0_14px_40px_rgba(23,23,23,0.04)]">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-5 py-4">
          <div><div className="flex items-center gap-2"><Users className="h-5 w-5" /><h2 className="text-lg font-bold">Membrii organizației</h2></div><p className="mt-1 text-sm text-muted-foreground">Rolul și accesul pot fi diferite pentru fiecare locație.</p></div>
            ...
            {filteredMembers.map((group) => {
              ...
              return (
                <div key={group.user_id} className="flex flex-col gap-4 px-5 py-4 transition hover:bg-secondary/10 lg:flex-row lg:items-center">
                  ...
                  <button type="button" onClick={() => openMember(group)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-border bg-background px-4 py-2.5 text-sm font-semibold hover:bg-secondary">Gestionează accesul <ChevronRight className="h-3.5 w-3.5" /></button>
                </div>
              );
            })}
          </div>
            ...
            {filteredInvitations.map((invitation) => {
              ...
              return (
                <div key={invitation.id} className="flex flex-col gap-3 px-5 py-4 lg:flex-row lg:items-center">
                  ...
                  <div className="min-w-0 flex-1 lg:max-w-[420px]"><div className="text-sm font-semibold">{invitedLocations.length === locationOptions.length && locationOptions.length > 1 ? "Toate locațiile actuale" : `${invitedLocations.length} ${invitedLocations.length === 1 ? "locație" : "locații"}`}</div>{invitedLocations.length > 0 && invitedLocations.length !== locationOptions.length && <div className="mt-1 truncate text-xs text-muted-foreground">{invitedLocations.join(" · ")}</div>}</div>
                  ...
                  <button type="button" disabled={saving} onClick={() => revokeInvitation(invitation.id)} className="shrink-0 rounded-full border border-border px-3 py-2 text-sm font-semibold text-destructive hover:bg-secondary disabled:opacity-50">Revocă</button>
                </div>
              );
            })}
          </div>
      ...
      <Drawer open={inviteOpen} title="Invită utilizator" subtitle="Alege rolul și una sau mai multe locații înainte de trimiterea invitației." onClose={() => setInviteOpen(false)}>
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="mt-2 space-y-2">
              {ROLE_ORDER.map((role) => {
                const descriptions = {
                  organization_owner: "Acces complet la organizație și la toate locațiile actuale.",
                  location_manager: "Poate gestiona conținutul și operațiunile locațiilor selectate.",
          ...
          <div>
            <div className="flex items-center justify-between gap-3">
              <div><div className="text-sm font-semibold">Acces la locații</div><p className="mt-1 text-xs text-muted-foreground">Selectează una sau mai multe locații.</p></div>
            ...
            <div className="mt-3 space-y-2">
              ...
              {form.role === "organization_owner" && <p className="rounded-xl bg-secondary/40 px-3 py-2 text-xs leading-relaxed text-muted-foreground">Ownerul este asociat automat tuturor locațiilor actuale și viitoare ale organizației.</p>}
            </div>
          </div>
      ...
      <Drawer open={Boolean(memberOpen)} title={memberOpen?.user_name || memberOpen?.user_email_masked || "Acces utilizator"} subtitle="Bifează locațiile și stabilește rolul separat pentru fiecare." onClose={() => setMemberOpen(null)}>
        {memberOpen && (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-3"><div><div className="flex items-center gap-2"><Building2 className="h-4 w-4" /><h3 className="text-base font-bold">Acces pe locații</h3></div><p className="mt-1 text-sm text-muted-foreground">Locațiile nebifate nu vor mai fi accesibile.</p></div>{locationOptions.length > 1 && <button type="button" onClick={selectAllMemberLocations} className="text-xs font-semibold underline underline-offset-4">{locationOptions.every((location) => memberAssignments[location.id]?.selected) ? "Șterge selecția" : "Selectează toate"}</button>}</div>
            ...
            {message && <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs text-amber-900">{message}</div>}
            <div className="rounded-2xl border border-border bg-secondary/25 p-3 text-sm leading-relaxed text-muted-foreground">Ownerii organizației primesc acces la toate locațiile actuale și viitoare. Managerii și membrii primesc doar locațiile selectate.</div>
            <button type="button" disabled={saving} onClick={saveMemberAccess} className="inline-flex h-11 w-full items-center justify-center rounded-full bg-foreground px-5 text-sm font-semibold text-background disabled:opacity-50">{saving ? "Se salvează..." : "Salvează accesul"}</button>
          </div>
        )}
      </Drawer>
    </div>
