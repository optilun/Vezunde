...
function organizationStatus(organization) {
  if (organization?.status === "suspendata" || organization?.status === "suspended") return { label: "Suspendata", className: "bg-red-100 text-red-800" };
  if (organization?.status === "inactiva" || organization?.status === "inactive") return { label: "Inactiva", className: "bg-secondary text-muted-foreground" };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 p-4" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-border bg-card shadow-xl" role="dialog" aria-modal="true" aria-labelledby="provider-settings-confirm-title">
        ...
        <div className="flex justify-end gap-2 border-t border-border bg-secondary/20 px-5 py-4">
          ...
          >
            {isClose ? <Archive className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}
            {isClose ? "Continua cu solicitarea" : "Continua cu solicitarea"}
          </button>
        </div>
      </div>
    </div>
          ...
          action={(
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <span className="rounded-full bg-secondary px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">
                {organization?.public_visibility_status === "public" ? "Profil public" : "Profil in pregatire"}
              </span>
            </div>
          )}
        />
