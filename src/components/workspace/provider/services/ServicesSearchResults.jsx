...
export default function ServicesSearchResults({ query, results, selected, approvedSelected, serviceUnitMap, activeUnits, prerequisites, disabled, onToggleService }) {
  return (
      ...
      {results.length > 0 ? results.map(({ section, item }) => {
        ...
        return (
          <div key={`${section.key}:${item.id}`}>
            ...
            <ServiceRow item={item} selected={selected} approvedSelected={approvedSelected} prerequisite={prerequisites[item.id]} unitKey={unitKey} disabled={disabled} onToggle={onToggleService} />
          </div>
        );
