...
export default function ProviderLeadInbox({ locationId, location }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const inboxResponse = await base44.functions.invoke("providerLeadInboxOps", { action: "list", location_id: locationId, status: filter === "all" ? "" : filter, limit: 100 });
  ...
  const markViewed = async (leadId) => {
    ...
    setError("");
    try {
      const response = await base44.functions.invoke("providerLeadInboxOps", { action: "mark_viewed", location_id: locationId, lead_id: leadId });
      const updated = responseData(response).lead;
      setData((current) => {
        if (!current) return current;
        const leads = current.leads.map((lead) => (lead.id === updated.id ? { ...lead, status: updated.status } : lead)).filter((lead) => filter === "all" || lead.status === filter);
        return { ...current, leads, counters: { ...current.counters, new: Math.max(0, Number(current.counters?.new || 0) - 1), viewed: Number(current.counters?.viewed || 0) + 1 } };
      });
    } catch (markError) {
  ...
  const submitResponse = async (leadId, responseType) => {
    ...
    setError("");
    try {
      const response = await base44.functions.invoke("providerLeadResponseOps", { action: "submit", location_id: locationId, lead_id: leadId, response_type: responseType });
      const result = responseData(response);
      setResponsesByLead((current) => ({ ...current, [leadId]: result.response }));
      setData((current) => current ? { ...current, leads: current.leads.map((lead) => (lead.id === leadId ? { ...lead, status: result.lead_status } : lead)).filter((lead) => filter === "all" || lead.status === filter) } : current);
    } catch (responseError) {
