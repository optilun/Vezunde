...
export default function ProviderLeadChat({ leadId, locationId, enabled, responseType }) {
  ...
  return (
    <div className="mt-5 rounded-2xl border border-primary/15 bg-primary/5 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="inline-flex items-center gap-2 text-xs font-bold text-foreground"><MessageCircle className="h-4 w-4 text-primary" /> Chat VIASEE · Pro</p>
          <p className="mt-1 max-w-xl text-[11px] leading-relaxed text-muted-foreground">Clientul deschide conversația. Telefonul, emailurile și linkurile sunt blocate în mesaje.</p>
        </div>
      ...
      {(opened || closed) && (
        <>
          <div className="mt-4 max-h-80 space-y-2 overflow-y-auto rounded-xl border border-border bg-background p-3">
            {messages.length === 0 ? <p className="py-5 text-center text-xs text-muted-foreground">Conversația este deschisă, dar nu există mesaje.</p> : messages.map((item) => (
          ...
          {closed && <p className="mt-3 inline-flex items-center gap-2 text-xs text-muted-foreground"><LockKeyhole className="h-3.5 w-3.5" /> Conversația este închisă. Clientul o poate redeschide dacă leadul rămâne eligibil.</p>}
        </>
      )}
    </div>
  );
