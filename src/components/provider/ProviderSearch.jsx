...
export default function ProviderSearch({ onClaim, onNew }) {
  ...
  const handleClaim = (location) => {
    // Cautarea si confirmarea locatiei raman publice. Autentificarea este ceruta
    // dupa alegerea relatiei, astfel incat toate punctele de intrare au acelasi flux.
    onClaim(location);
  };
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="font-semibold flex items-center gap-1.5">
                  {loc.name}
                  {loc.status_label === "Verificat de VIASEE" && <BadgeCheck className="w-4 h-4 text-primary" />}
                </div>
                {loc.organization_name && <div className="text-xs text-muted-foreground">{loc.organization_name}</div>}
