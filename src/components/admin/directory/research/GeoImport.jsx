...
export default function GeoImport() {
  ...
  return (
    <div className="max-w-3xl">
      <h2 className="font-heading font-bold">Geografie Romania — import SIRUTA</h2>
      ...
      {preview && <GeoImportPreview preview={preview} />}

      {preview && preview.all_pass && !result && (
      ...
      {preview && !preview.all_pass && (
        <p className="mt-4 text-sm text-destructive font-semibold">Importul este blocat: validarile critice nu trec.</p>
      )}

      {progress && (
        <p className="mt-4 text-sm text-muted-foreground">Se importa... {progress.done} / {progress.total} randuri</p>
      )}
      {result && (
