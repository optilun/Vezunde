...
export default function DirOpsProfiles() {
  ...
  return (
    <AdminCard className="p-5">
      {!locations && <p className="text-muted-foreground text-sm">Se incarca...</p>}
      {locations && locations.length === 0 && (
        <EmptyState icon={Building2} title="Niciun profil in director." subtitle="Profilurile create prin fluxul Adauga locatie vor aparea aici." />
      )}
      {locations && locations.length > 0 && (
        <div className="space-y-2">
          {locations.map((l) => {
            ...
            return (
              <div key={l.id} className="bg-secondary/50 border border-border rounded-xl p-4 flex flex-wrap items-center gap-3">
                ...
                <span className={`text-xs font-semibold px-2 py-1 rounded-full ${pcs === "verified" ? "bg-green-100 text-green-800" : pcs === "claimed" ? "bg-blue-100 text-blue-800" : pcs === "suspended" ? "bg-red-100 text-red-800" : "bg-card text-foreground"}`}>
                  {PCS_LABELS[pcs]}
                </span>
                <div className="flex gap-2">
                  {pcs !== "verified" && (
                    <button onClick={() => setAction({ locationId: l.id, type: "verify" })} className="text-xs px-3 py-1.5 rounded-md bg-card border border-border hover:bg-accent">Verifica profil</button>
                  )}
                  {pcs !== "suspended" && (
                    <button onClick={() => setAction({ locationId: l.id, type: "suspend" })} className="text-xs px-3 py-1.5 rounded-md bg-card border border-border hover:bg-accent text-destructive">Suspenda</button>
                  )}
                </div>
              </div>
            );
