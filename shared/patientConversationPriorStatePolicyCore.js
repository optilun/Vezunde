...
export const PATIENT_CONVERSATION_PRIOR_STATE_POLICY_VERSION =
...
const REDACTION_MARKER_PATTERN = /\[(?:email|telefon|identificator) eliminat\]/gi;
...
function controlledPriorText(value, maxLength) {
  const redacted = redactPatientConversationText(value, maxLength)
    .replace(/\s{2,}/g, " ")
    .trim();
  const meaningful = redacted.replace(REDACTION_MARKER_PATTERN, "").trim();
  return meaningful ? redacted : "";
}

function controlledLocalityText(value, maxLength) {
  const redacted = controlledPriorText(value, maxLength);
  if (!redacted || REDACTION_MARKER_PATTERN.test(redacted)) {
...
function sanitizedPriorFacts(value) {
  ...
  return {
    ...
    symptom_onset: controlledPriorText(facts.symptom_onset, 240),
    symptom_duration: controlledPriorText(facts.symptom_duration, 240),
    symptom_pattern: controlledPriorText(facts.symptom_pattern, 400),
    desired_timing: controlledPriorText(facts.desired_timing, 240),
    ...
    investigation_reference_text: controlledPriorText(
      facts.investigation_reference_text,
      500,
    ),
    repair_details: controlledPriorText(facts.repair_details, 500),
    user_constraints: unique(facts.user_constraints, 8, 240)
      .map((value) => controlledPriorText(value, 240))
      .filter(Boolean),
  };
}

export function sanitizePatientConversationPriorState(value) {
  ...
  return {
    ...
    need_summary: controlledPriorText(value.need_summary, 500),
